...
//    let dbRef = Database.database().reference(fromURL: "https://snap-chat-clone-part-2.firebaseio.com")
    ...
    @IBAction func signUpPressed(_ sender: UIButton) {
        ...
        if email == "" || password == "" || name == "" || verifiedPassword == "" {
            ...
            Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
                ...
                if error == nil {
                    Auth.auth().currentUser?.createProfileChangeRequest().displayName = email
                    Auth.auth().currentUser?.createProfileChangeRequest().commitChanges { (error) in
                        
                    }
                self.dbRef.child("Users").child((Auth.auth().currentUser?.uid)!).setValue(["username": (Auth.auth().currentUser?.uid)!])
                    print("\(Auth.auth().currentUser?.uid)")
                    let alertController = UIAlertController(title: "Thanks for creating an account!", message: "Enjoy!", preferredStyle: .alert)
                    let defaultAction = UIAlertAction(title: "OK", style: .cancel, handler: nil)
                    alertController.addAction(defaultAction)
