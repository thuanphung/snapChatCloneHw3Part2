...
class CurrentUser {
    ...
    func getReadPostIDs(completion: @escaping ([String]) -> Void) {
        ...
        dbRef.observeSingleEvent(of: .value) { (DataSnapshot) in
            if DataSnapshot.exists() {
                let storedVal: [String: AnyObject]
                storedVal = DataSnapshot.value as! [String : AnyObject]
                for (key, value)  in storedVal {
    ...
    func addNewReadPost(postID: String) {
        dbRef.child("Users").child("\(id)").childByAutoId().setValue(postID)
    }
