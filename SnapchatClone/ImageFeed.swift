            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                ...
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        if let postDictionary = posts[postKey] as? [String:AnyObject] {
                            
                            
                            let post = Post(
                                id: postKey,
                                username: "\(user)",
                                postImagePath: "\(postDictionary["imagePath"])",
                                thread: "\(postDictionary["thread"])",
                                dateString: "\(postDictionary["date"])",
                                read: ids.contains(postKey))
